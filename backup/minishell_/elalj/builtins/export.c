...
//int	export_valid_args(t_cmd *cmd, int I)
//{
//	int i;
//	int	n_arg;
...
void	my_export(t_cmd *cmd, t_data *data, int I, int j)
{
	int	i;
	int	check;
	...
	check = 0;
	i = 0;
	if (cmd[I].args[j] && (!cmd[I].args[j + 1] || cmd[I].args[j + 1][0] == '\0'))
	{
		while (data->c_env[i] != NULL)
		{
			printf("declare -x ");
